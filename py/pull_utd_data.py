...
def pull_utd_data():
    ...
    while year <= 2019:
        ...
        panda_table = pd.read_html(str(table), header=1)[0]
        panda_table.rename(columns = {"date" : "date","date.1" : "time"},inplace=True)
        strings_to_drop = ['Premier','FA','League','date']
        panda_table = panda_table[~panda_table.date.str.contains('|'.join(strings_to_drop))]
        #print panda_table
